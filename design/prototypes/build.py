...
a{color:var(--accent)}
.wrap{max-width:1360px;margin:0 auto;padding:0 24px;width:100%}

/* ---------- masthead: the existing logo, on the black it was drawn for ---------- */
.masthead{background:#000;line-height:0}
.logo{display:block;max-width:100%;height:auto;width:591px;margin:0}

/* ---------- nav ---------- */
.nav{border-top:1px solid var(--rule-strong);border-bottom:1px solid var(--rule);background:var(--panel)}
...
.supports{font:12px/1.6 var(--mono);color:var(--ink-dim);margin:10px 2px 0;max-width:120ch}
...
.legend{font:12px/1.6 var(--mono);color:var(--ink-dim);margin:12px 2px 0;max-width:120ch}
