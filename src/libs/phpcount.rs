//! PHPCount - hit counter for CrackStation.
//!
...
//!
//! It used to be a bare `SHA256(pageID ‖ IP)`. Both inputs are public and
//! low-entropy: the page identifier is one of ten compile-time constants in an
//! AGPL-licensed repository and the address is 32 bits for an IPv4 visitor, so the
//! whole table could be reversed by sweeping the space — measured during the audit at
//! about two seconds on one GPU, and a *targeted* "was this address here?" is a single
//! hash and a lookup. The table was a rolling 30-day, per-page, timestamped list of
//! visitor IP addresses while documenting itself as privacy-preserving.
//!
...
//!   in `cshits` are untouched; only the "have I seen this visitor" test resets.
...
impl PhpCountService {
    ...
    ///
    /// This runs on **every counted page view**, and on MyISAM it holds a **table-level
    /// write lock** -- so its cost is not per-page, it serialises the whole site.
    ///
    /// `csnodupes` now carries `KEY (time)` alongside `PRIMARY KEY (ids_hash)`, which
    /// makes the sweep an index range scan over just the expired rows -- usually none.
    /// Production was measured through the whole progression on 2026-09-01, with a 23 MB
    /// data file holding ~200k live rows:
    ///
    /// | state | sweep |
    /// | --- | --- |
    /// | full scan, 8 MB of holes | 20 ms |
    /// | after `OPTIMIZE TABLE` | 14 ms |
    /// | after `ADD KEY (time)` | 1 ms |
    ///
    /// At 20 ms under a table lock that was a ceiling of roughly 50 counted page views a
    /// second for the entire site. The index is what removed it; `OPTIMIZE` alone only
    /// shrank the scan.
    ///
    /// So if this is slow again, the first question is whether the index is still there
    /// and still being used -- a schema restored from an older dump would not have it:
    ///
    /// ```sql
    /// SHOW INDEX FROM csnodupes;                     -- expect a `time` key
    /// EXPLAIN DELETE FROM csnodupes WHERE time < 1;  -- type=range, not type=ALL
    /// SHOW TABLE STATUS LIKE 'csnodupes';            -- Data_free large while Rows is small
    /// ```
    ///
    /// The second question is `Data_free`. MyISAM flags a deleted row and links its slot
    /// onto a free list; the data file never shrinks. That no longer costs a full scan
    /// per request, but it still inflates any scan the optimiser does choose -- and it
    /// will choose one when a large enough fraction of rows is expired at once, such as
    /// after an outage. `OPTIMIZE TABLE csnodupes;` reclaims the holes, and needs
    /// repeating, since they come back as rows age out.
    ///
    /// Still available if it ever matters: `ALTER TABLE csnodupes ENGINE=InnoDB;` removes
    /// the site-wide write lock entirely. Not done -- see
    /// `schema-pins-hit-counter-tables-to-myisam...` in the audit for why the engine
    /// stays.
